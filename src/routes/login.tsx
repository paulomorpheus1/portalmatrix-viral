    ...
    <div className="min-h-screen flex items-center justify-center px-4 matrix-grid">
      <form onSubmit={onSubmit} className="glass rounded-xl p-8 w-full max-w-sm space-y-4">
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Acesse o Portal Matrix</p>
        </div>
        ...
        <Button type="submit" className="w-full neon-border" disabled={loading} style={{ background: "var(--gradient-primary)" }}>
          {loading ? "Entrando…" : "Entrar"}
        </Button>
        <p className="text-xs text-center text-muted-foreground">Sem conta? <Link to="/signup" className="text-accent">Criar agora</Link></p>
      </form>
    </div>
  );
}
